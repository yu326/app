function TopicConfigPartAuthConfig(accountjson,datajson,target){
	var _this = this;
		...
		switch(item)
		{
			...
			case "createdtime":
				if(paramsjson.filter[item].limit.length > 0){
					...
					$("#topic_authconfig_facet_range_createdtime_start").val(minv);
					$("#topic_authconfig_facet_range_createdtime_end").val(maxv);
					if(minv == "" && maxv == ""){
						$("#topic_authconfig_facet_range_createdtime_all").attr("checked", true);
					}
					else{
						setTimeSelectMsg("topic_authconfig_facet_range", minv, maxv, true); 
					}
					changeTimeBorderColor("topic_authconfig_facet_range_html", "createdtime"); 
					$("#topic_authconfig_facet_range_html").attr("checkedtimetype", "createdtime");
				}
				else{
					$("#topic_authconfig_facet_range_createdtime_all").attr("checked", true);
				}
			...
			case "nearlytime":
				if(paramsjson.filter[item].limit.length > 0 && paramsjson.filter[item].limit[0].value.start!=undefined){
					var facet_range_prefix = "topic_authconfig_facet_range";
					...
					changeTimeBorderColor("topic_authconfig_facet_range_html", "nearlytime"); 
					$("#topic_authconfig_facet_range_html").attr("checkedtimetype", "nearlytime");
				}
				break;
			case "beforetime":
				if(paramsjson.filter[item].limit.length > 0 && paramsjson.filter[item].limit[0].value.start != undefined){
					...
					changeTimeBorderColor("topic_authconfig_facet_range_html", "beforetime"); 
					$("#topic_authconfig_facet_range_html").attr("checkedtimetype", "beforetime");
				}
				break;
			case "untiltime":
				if(paramsjson.filter[item].limit.length > 0 && paramsjson.filter[item].limit[0].value.start != undefined){
					...
					changeTimeBorderColor("topic_authconfig_facet_range_html", "untiltime"); 
					$("#topic_authconfig_facet_range_html").attr("checkedtimetype", "untiltime");
				}
				break;
			...
			case "facetlimit":
				if(paramsjson.facet.limit.length>0) {
					var strhtml ="";
					$.each(paramsjson.facet.limit, function(li, litem){
						//当limitcontrol == 0时不可修改 去掉点击事件
						var facetname = "";
						if(paramsjson.filter[litem.value]!=undefined){
							facetname = paramsjson.filter[litem.value].label
						}
						else{
							facetname = getDisplayName(litem.value);
						}

						if(facetname != ""){
							var isclick = "";
							if(_this.accountJson.facet.limitcontrol != 0){
								isclick = "<a name='cancleselectedvaluetextobj' value='"+litem.value+"' class='cancleitem'>×</a>";
							}
							strhtml+="<span class='fixedselwordsbox '><span class='facetlimititem' code=" + litem.value + " >" + facetname + "</span>"+isclick+"</span>";	
						}
					});
					$("#selected"+item).html(strhtml);
				}
	...
	this.init = function(){
		...
		timeDisplayHtml("topic_authconfig");
		...
		if(paramsjson.facet.filterlimit.limit.length == 0){
			paramsjson.facet.filterlimit.limit = _this.accountJson.facet.filterlimit.limit;
		}
	...
	this.getParams = function(){
		...
		var timetype = $("#topic_authconfig_facet_range_html").attr("checkedtimetype"); //最后一次选择的时间控件
		for(var item in paramsjson.filter)
		{ 
			var htmlitem = item;
			switch(item){
			...
			switch(item)
			{
				...
				case "ancestor_emoAccount":
					...
					break;
				case "createdtime":
					var allchk = $("#topic_authconfig_facet_range_createdtime_all").prop("checked");
					if(timetype == "createdtime" && !allchk){
						var cts = $("#topic_authconfig_facet_range_createdtime_start").val();
						var cte = $("#topic_authconfig_facet_range_createdtime_end").val();
				...
				case "nearlytime":
					if(timetype == "nearlytime"){
						var timevalue = getnearlytimeValue("topic_authconfig_facet_range");
				...
				case "beforetime":
					if(timetype == "beforetime"){
						var timevalue = getbeforeTimeValue("topic_authconfig_facet_range");
				...
				case "untiltime":
					if(timetype == "untiltime"){
						var timevalue = getuntiltimeValue("topic_authconfig_facet_range");
			...
			if(chklimit == -1){
				alert(paramsjson.filter[alertitem].label+"不允许修改");
				return false;
			}
			else if(chklimit == 0){
				alert(paramsjson.filter[alertitem].label+"值超出范围");
				return false;
			}
		}
	...
	this.render = function(){
		// 请求HTML
		$.ajax({type: "GET", url: config.sitePath+"parts/topic_config_part_authconfig.html", async:false,  // 同步请求
			dataType: "html", 
			success: function(data){
 ...
 */
TopicConfigPartAuthConfig.prototype.cancleUser = function(elm){
	$(elm).parent().remove();
	/*if(opt == "end"){
...
TopicConfigPartAuthConfig.prototype.cancleArea= function(elm,filtername){
