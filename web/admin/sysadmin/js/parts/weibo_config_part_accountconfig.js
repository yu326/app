function WeiboConfigPartAccountConfig(datajson,target){
	var _this = this;
	...
	this.init = function(){
		 ...
		 */
		timeDisplayHtml("weibo_accountconfig");
			...
			switch(item)
			{
				...
				case "createdtime":
					if(paramsjson.filter[item].limit.length > 0){
						...
						$("#weibo_accountconfig_facet_range_createdtime_start").val(minv);
						$("#weibo_accountconfig_facet_range_createdtime_end").val(maxv);
						if(minv == "" && maxv == ""){
							$("#weibo_accountconfig_facet_range_createdtime_all").attr("checked", true);
						}
						else{
							setTimeSelectMsg("weibo_accountconfig_facet_range", minv, maxv, true); 
						}

						setTimeSelectMsg("weibo_accountconfig", minv, maxv, true); 
						changeTimeBorderColor("weibo_accountconfig_facet_range_html", "createdtime"); 
						$("#weibo_accountconfig_facet_range_html").attr("checkedtimetype", "createdtime");
					}
					else{
						$("#weibo_accountconfig_facet_range_createdtime_all").attr("checked", true);
					}
				...
				case "nearlytime":
					if(paramsjson.filter[item].limit.length > 0 && paramsjson.filter[item].limit[0].value.start != undefined){
						var facet_range_prefix = "weibo_accountconfig_facet_range";
						...
						changeTimeBorderColor("weibo_accountconfig_facet_range_html", "nearlytime"); 
						$("#weibo_accountconfig_facet_range_html").attr("checkedtimetype", "nearlytime");
					}
					break;
				case "beforetime":
					if(paramsjson.filter[item].limit.length > 0 && paramsjson.filter[item].limit[0].value.start != undefined){
						...
						changeTimeBorderColor("weibo_accountconfig_facet_range_html", "beforetime"); 
						$("#weibo_accountconfig_facet_range_html").attr("checkedtimetype", "beforetime");
					}
					break;
				case "untiltime":
					if(paramsjson.filter[item].limit.length > 0 && paramsjson.filter[item].limit[0].value.start != undefined){
						...
						changeTimeBorderColor("weibo_accountconfig_facet_range_html", "untiltime"); 
						$("#weibo_accountconfig_facet_range_html").attr("checkedtimetype", "untiltime");
					}
					break;
				default:
	...
	this.getParams = function(){
		...
		var timetype = $("#weibo_accountconfig_facet_range_html").attr("checkedtimetype"); //最后一次选择的时间控件
		var paramsjson = _this.dataJson;
		for(var item in paramsjson.filter)
		{
			var htmlitem = item;
			...
			switch(item)
			{
				...
				case "sourceid":
					...
					break;
				case "createdtime":
					var allchk = $("#weibo_accountconfig_facet_range_createdtime_all").prop("checked");
					if(timetype == "createdtime" && !allchk){
						var cts = $("#weibo_accountconfig_facet_range_createdtime_start").val();
						var cte = $("#weibo_accountconfig_facet_range_createdtime_end").val();
				...
				case "nearlytime":
					if(timetype == "nearlytime"){
						var timevalue = getnearlytimeValue("weibo_accountconfig_facet_range");
				...
				case "beforetime":
					if(timetype == "beforetime"){
						var timevalue = getbeforeTimeValue("weibo_accountconfig_facet_range");
				...
				case "untiltime":
					if(timetype == "untiltime"){
						var timevalue = getuntiltimeValue("weibo_accountconfig_facet_range");
	...
	this.render = function(){
		// 请求HTML
		$.ajax({type: "GET", url: config.sitePath+"parts/weibo_config_part_accountconfig.html", async:false,  // 同步请求
			dataType: "html", 
