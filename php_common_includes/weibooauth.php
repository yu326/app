...
class SaeTClient
{
	 ...
	 * @return array
	 */
	function public_timeline( $count = 20, $base_app = NULL )
	{
		$params = array();
...
*/
	/**
	 * V2  注：接口升级后：uid与screen_name只能为当前授权用户；
	...
	return $this->request_with_uid( 'https://api.weibo.com/2/statuses/user_timeline.json' ,  $uid_or_name , $page , $count , NULL , true, $params );
	}
	 ...
	 * @return array
	 */
	function weibo_by_key($q, $page = 1 , $count = 20 , $since_id = NULL, $max_id = NULL)
	{
		//$this->urlencode($q);
		$params = array();
		$params["q"]=$q;
...
//request_with_pager( $url , $page = false , $count = false , $params = array() )
		return $this->request_with_pager( 'https://c.api.weibo.com/2/search/statuses/limited.json' ,  $page , $count ,  $params );
	}
	...
	function repost_by_me( $uid, $page = 1 , $count = 20, $since_id = NULL, $max_id = NULL )
{
	...
	function repost_timeline( $sid, $page = 1 , $count = 20, $since_id = NULL, $max_id = NULL )
	{
		...
		return $this->request_with_pager( 'https://api.weibo.com/2/statuses/repost_timeline.json', $page , $count , $params );
	}
	...
	return $this->oauth->get('https://c.api.weibo.com/2/comments/show.json' , $params );
	...
	function weekly_trends( $base_app = 1 )
	{
		...
		return $this->oauth->get( 'http://api.t.sina.com.cn/treznds/weekly.json' , $params );
	}
...
class SaeTOAuth {
	...
	function get($url, $parameters = array()) {
		...
		if ($this->format === 'json' && $this->decode_json) {
			$response = preg_replace('/("id":)(\d{9,})/i', '${1}"${2}"', $response); //add by jiabei
