...
class SaeTOAuthV2 {
	 ...
	 * @ignore
	 */
	public $host = "http://c.api.weibo.com/";
	/**
	 * Set timeout default.
	 *
	 * @ignore
    ...
    function oAuthRequest($url, $method, $parameters, $multi = false) {
		global $logger;
		$logger->info(__FILE__.__LINE__."url is ::::::".var_export($url,true));
        if (strrpos($url, 'http://') !== 0 && strrpos($url, 'https://') !== 0) {
			if($url == "datapush/status"){
				$url = "{$this->host}{$url}";
			}else{
        ...
        switch ($method) {
        case 'GET':
            //http_build_query会把中文进行urlencode转换
            $url = $url . '?' . http_build_query($parameters);
			$logger->info(__FILE__.__LINE__."url22222 is ::::::".var_export($url,true));
	...
	function http($url, $method, $postfields = NULL, $headers = array()) {
        ...
        //var_dump(__FILE__.__LINE__, $response);
		$this->http_code = curl_getinfo($ci, CURLINFO_HTTP_CODE);
		$this->http_info = array_merge($this->http_info, curl_getinfo($ci));
		$this->url = $url;

		if ($this->debug) {
            var_dump(__FILE__.__LINE__, $url);
			echo "=====post data======\r\n";
			var_dump($postfields);
...
class SaeTClientV2
{
	 ...
	 * 搜索包含某个关键词的微博
	 *
	 * 对应API：{@link http://open.weibo.com/wiki/C/2/search/statuses/limited/en statuses/limited}
	 *
	 * @access 收费
	 * @param string $q Keyword for searching.It must be encoded by URLencoding。
	 * @param int $count 单页返回的记录条数，默认为50。
	 * @param int $page 返回结果的页码，默认为1。
	 * @return array
	 */
	function limited($q, $page = 1, $count = 50)
	{
		$params = array();
		$params['q'] = $q;
		$params['count'] = intval($count);
		$params['page'] = intval($page);
		return $this->oauth->get('statuses/limited', $params);
	}

	/**
	 * 创建检索历史任务
	 *
	 * 对应API：{@link http://open.weibo.com/wiki/C/2/search/statuses/historical/create statuses/historical/create}
	 *
	 * @access 收费
     * @param int64  $starttime 搜索范围起始时间，取值为时间戳（精确到毫秒）。
     * @param int64  $endtime 搜索范围结束时间，取值为时间戳（精确到毫秒，最晚不能晚过昨天的最后一秒）
	 * @param string $q 检索的关键词。最多指定1000个关键词，每个关键词长度不超过24个中文字符或英文字符，由英文半角逗号分隔。匹配上任意关键词的微博都会被检索到。必须进行URLencode。
	 * @return array
	 */
	function historical_create($q, $starttime, $endtime)
	{
		$params = array();
		$params['q'] = $q;
        ...
        //新接口地址
        $this->oauth->newAPIhost();
		return $this->oauth->post('statuses/historical/create', $params);
	}
    /*
     * add by bert end
     * */
	 ...
	 *  添加订阅关键词
	 *
	 * 对应API：{@link https://open.weibo.com/wiki/C/2/subscribe/update_subscribe}
	 *
	 *  @param int $subid 订阅号。
	 *  @param string $add_keywords 要添加的订阅关键词。
	 *  @param string $del_keywords 要删除的订阅关键词。
	 *  @param int $add_uids 要添加的订阅id。
	 *  @param int $del_uids 要删除的订阅id。
	 *  @return array
	 * */
	function add_keywords($subid,$add_keywords,$del_keywords,$add_uids,$del_uids){
		$params = array();
		$params['subid'] = $subid;
		$params['add_keywords'] = $add_keywords;
		$params['del_keywords'] = $del_keywords;
		$params['add_uids'] = $add_uids;
		$params['del_uids'] = $del_uids;
		return $this->oauth->post( 'subscribe/update_subscribe', $params );
	}
	/*
	 *  获取订阅推送的数据
	 *
	 * 对应API：{@link http://open.weibo.com/wiki/C/2/datapush/status}
	 *
	 *  @param int $subid 订阅号。
	 *  @param int $since_id 断点id。
	 *  @return array
	 * */
	function datapush($subid,$since_id){
		global $logger;
		$params = array();
		$params['subid'] = $subid;
		$params['since_id'] = $since_id;
		$logger->info("the  area  is".var_export($this->oauth,true));
		return $this->oauth->get( 'datapush/status', $params );
	}
...
class SaeTClientV2Biz
{
	...
	function __construct( $akey, $skey, $access_token, $refresh_token = NULL)
	{
		$this->oauth = new SaeTOAuthV2( $akey, $skey, $access_token, $refresh_token );
        //商业接口地址
        $this->oauth->host = "https://c.api.weibo.com/2/";
	}
	 ...
	 * @access 收费
	 * @param string $q //Keyword for searching.It must be encoded by URLencoding。
	 * @param int $count //The returned count for every page.Default is 10.The max value is 50.
	 * @param int $page //The returned page.Default is 1。
	 * @param string $ids //Search the statuses which are created by the uids specified.The uids should be separated by ~, and are limited by 50 uids per call。
	 * @param string $sort //Return the weibos by the sort type.time : reverse order by time; hot : order by hot; fwnum: reverse order by repost count; cmtnum: reverse order by comment count; Default is time。
     * @param int $starttime //Begin time. Unix timestamp format.
     * @param int $endtime //End time. Unix timestamp format.
     * @param int $dup //Whether returns the similar weibos. 0: no; 1: yes; Default is 1.
     * @param int $onlynum //Whether only returns the total number. 0: no; 1: yes;Default is 0.
	 * @return array
	 */
	function limited($q, $page = 1, $count = 50, $ids = NULL, $sort = NULL, $starttime = 0, $endtime = 0, $dup = 0, $onlynum = 0, $antispam = 0)
	{
		$params = array();
		$params['q'] = $q;
		$params['count'] = intval($count);
		...
		return $this->oauth->get('search/statuses/limited', $params);
	}
	...
	function historical_create($q, $starttime, $endtime)
	{
		...
		$params['starttime'] = $starttime;
		$params['endtime'] = $endtime;
		return $this->oauth->post('search/statuses/historical/create', $params);
	}

	/**
	 * 返回一条微博的全部转发微博列表
	 *
	 * 对应API：{@link https://c.api.weibo.com/2/statuses/repost_timeline/all statuses/repost_timeline/all}
	 *
	 * @access 收费
	 ...
	 * @return array
	 */
	function repost_timeline_all($id, $page = 1, $count = 20)
	{
		$params = array();
		$params['id'] = $id;
		$params['count'] = $count;
		$params['page'] = $page;
		return $this->oauth->get('statuses/repost_timeline/all', $params);
	}

	/**
	 * 返回一条微博的全部评论列表
	 *
	 * 对应API：{@link https://c.api.weibo.com/2/comments/show/all comments/show/all}
	 *
	 * @access 收费
     * @param int64 $id 需要查询的微博ID。
     * @param int   $count 单页返回的记录条数，最大不超过200，默认为20
	 ...
	 * @return array
	 */
	function comments_show_all($id, $page = 1, $count = 20)
	{
		$params = array();
		$params['id'] = $id;
		$params['count'] = $count;
		$params['page'] = $page;
		return $this->oauth->get('comments/show/all', $params);
	}
	
	/**
	 * 获取用户发布的微博信息列表(只返回最近200条)
	 *
	 * 返回用户的发布的最近n条信息，和用户微博页面返回内容是一致的。此接口也可以请求其他用户的最新发表微博。
	 * <br />对应API：{@link http://open.weibo.com/wiki/2/statuses/user_timeline_batch statuses/user_timeline_batch}
	 * 
	 * @access public
	 * @param int $page 页码
	 * @param int $count 每次返回的最大记录数，最多返回200条，默认50。
	 * @param int $flag 用于筛选晋升职位。0用于返回晋升的职位和普通职位，2用于返回普通职位。默认为0。
	 * @param mixed $uid 指定用户UID或微博昵称
	 * @param int $base_app 是否基于当前应用来获取数据。1为限制本应用微博，0为不做限制。默认为0。
	 * @param int $feature 过滤类型ID，0：全部、1：原创、2：图片、3：视频、4：音乐，默认为0。
	 * @return array
	 */
	function user_timeline_by_id( $uid = NULL , $page = 1 , $count = 50 , $feature = 0,$base_app = 0, $flag = 0)
	{
		$params = array();
		$params['uids']=$uid;
		$params['base_app'] = intval($base_app);
		...
		$params['count'] = intval($count);
		$params['page'] = intval($page);
		$params['flag'] = intval($flag);
		return $this->oauth->get( 'statuses/user_timeline_batch', $params );
	}
	 ...
	 *
	 * 返回微博id对应的微博信息
	 * <br />对应API：{@link http://open.weibo.com/wiki/2/search/suggestions/users/biz}
	 *
	 * @access public
	 * @param string $q 指定用户昵称
	 * @return array
	 */
	function   get_userid($q,$count){
		$params = array();
		$params['q'] = $q;
		$params['count'] = $count;
		return $this->oauth->get( 'search/suggestions/users/biz',$params);
	}

	/*
	 * 根据一批微博id获取最新的转评喜欢数   （一次最多20个id，以“，”隔开）
	 *
	 * 返回微博id对应的转评喜欢数
	 *<br />对应API：{@link http://open.weibo.com/wiki/2/search/suggestions/users/biz}
	 *
	 * @access public
	 * @param int $ids 指定用户昵称
	 *@return array
	 * ***/
	function getNewRepost($ids){
		$params = array();
		$params['ids'] = $ids;
		return $this->oauth->get( 'statuses/count/biz', $params );
	}
}
