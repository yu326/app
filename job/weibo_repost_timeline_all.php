...
try {
    ...
    $task = getWaitingTask(TASKTYPE_SPIDER, TASK_REPOST_TREND);
...
exit;
//抓取
function crawling_repost_timeline_all($id, $page = 1, $count = 50)
{
    ...
    $weibos_info = $oAuthThirdBiz->repost_timeline_all($id, $page, $count);
    ...
    $logger->info(SELF . " - 调用weibo_repost_timeline_all花费时间：" . $apitimediff);
    $result = true;
...
function execute()
{
    ...
    foreach($idArr as $key=>$id){
        ...
        while (1) {
			//因为新浪接口返回数据为空，影响抓取数据入库，所以加入判断机制
			//     	根据返回总条数，计算出最大请求数$max_page，$succ_num成功请求次数，$fail_num失败请求次数 
			//根据接口数据，进行判断     
			//		接口数据为空的情况下，$fail_num+1会继续请求接口，如果$fail_num大于2，则结束任务。
			//      不为空，$succ_num+1,page+1，继续请求，当$succ_num>=$max_page时，结束任务
			//      若$max_page为整数，由于新浪返回数据有缺失，所以需要多抓取一次，所以此时$max_page+1，
            $s_time = microtime_float();
            $logger->debug(__FILE__.__LINE__." page ".var_export($page, true)." each_count".var_export($each_count, true));
            $weibos_info = crawling_repost_timeline_all($id, $page, $each_count);
            $logger->debug(__FILE__.__LINE__." weibos_info ".var_export($weibos_info, true));
            if($succ_num >$max_page || $fail_num>2 ){
                break;
            }
            ...
            if($max_page == '0'){
				$max_page = $weibos_info['0']['retweeted_status']['reposts_count']/$each_count;
				if(is_int($max_page)){
					$max_page = $max_page + 1;
				}
				else{
					$max_page = ceil($weibos_info['0']['retweeted_status']['reposts_count']/$each_count);
				}
			}
            ...
            $logger->debug(__FILE__.__LINE__." weibos_info after ".var_export($weibos_info, true));
//            $solr_r = addweibo($sourceid, $weibos_info, $isseed, 'weibo_repost_timeline_all'); //允许数据不全
            $solr_r = commonHandleData($sourceid, $weibos_info, $isseed, 'weibo_repost_timeline_all');
            unset($weibos_info);
