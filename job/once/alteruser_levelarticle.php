...
$totalnum = solr_select_conds(array('guid', 'verified_type','sourceid'), "sourceid:1+AND+verify:2", 0, 0);
if($end == 0){
	$end = $totalnum;
}
...
while(1){
	...
	$seqno = $start;
	$shift = 0;
	$qr = solr_select_conds(array('guid', 'verified_type','sourceid'), "sourceid:1+AND+verify:2", $start, $each_count, "created_at+asc");
	if($qr === false){
		...
		$rcnt = solr_select_conds(array('guid', 'verified_type','sourceid'), "sourceid:1+AND+verify:2", $start, $each_count);
		if($rcnt == 0){
			break;
		}
		$solrarticle = array();
		foreach($qr as $qi=>$rec){
			if(isset($rec['verified_type'])){
				$vt;
				if(is_array($rec['verified_type'])){
					$vt = $rec['verified_type'][0];
				}
				else{
					$vt = $rec['verified_type'];
				}
				switch($vt){
				case 200:
					$users_level = 1;
