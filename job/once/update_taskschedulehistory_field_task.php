...
function getTotalCount(){
	global $dsql,$logger;
	$totalCount = "select count(*) as totalcount from ".DATABASE_TASKSCHEDULEHISTORY." where tasktype = ".TASKTYPE_UPDATE." and task = ".TASK_SNAPSHOT."";
...
function getTaskScheduleHistoryInfo($startnum, $pagesize){
	global $dsql,$logger;
	$limit = "";
	...
	$sql = "select id, task, params from ".DATABASE_TASKSCHEDULEHISTORY." where tasktype = ".TASKTYPE_UPDATE." and task = ".TASK_SNAPSHOT." ".$limit."";
	...
	if(!$qr2){
		...
		while ($result = $dsql->GetArray($qr2, MYSQL_ASSOC)){
			...
			if(isset($params["taskparams"]["eventlist"])){
				$upsql = "update ".DATABASE_TASKSCHEDULEHISTORY." set task = ".TASK_EVENTALERT." where id= ".$result["id"]."";
...
for($i=0;$i< $totalcount; $i+=$persize ){
	...
	getTaskScheduleHistoryInfo($i, $persize);
	$gend_time = microtime_float();
	$getuser_time = $gend_time - $gstart_time;
	echo "per getTaskScheduleHistoryInfo time:".$getuser_time."\n";
}
