...
function execute() {
   ...
   // $last_id = implode(",",$ids);
   // $logger->info(__FILE__.__LINE__." the last_id is:".var_export($last_id,true));
   $ch = curl_init();
   $url = "http://c.api.weibo.com/datapush/status?subid=10719";
//    $logger->info(__FILE__.__LINE__."the  last_id is:".var_export($last_id,true));
   if(isset($last_id)){
...
unset($arr);
$solr_r = addweibo(1, $data, 1, 'datapush');
unset($data);
