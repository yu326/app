...
try{
	$task = getWaitingTask(TASKTYPE_ANALYSIS, TASK_COMMENTPATH);
	if(!empty($task)){
	...
	$task = getQueueTask($currentmachine, TASK_COMMENTPATH);
	if(empty($task)){
...
function execute()
{
    ...
    while (1)
    {
        ...
        if(!empty($articleguid)){
            ...
            $logger->debug('*** before calc_commenttrend memory:'.memory_get_usage());
            //$r = get_comments($seedweibo,$each_count,$task->taskparams->forceupdate);
			$r = calcTrendPath('comment_trend', array($tmpdata), true);
            $logger->debug('*** after calc_commenttrend memory:'.memory_get_usage());
            if($r['result'] == false){
				$err = array();
				$err['url'] = $url;
				$err['msg'] = $r['msg'];	
				$task->taskparams->errorurl[] = $err;
                $logger->error(SELF." - calc_commenttrend返回false ".var_export($err, true));
            }
            $e_time = microtime_float();
